...
class TriplaneNoiseGenerator(NoiseGenerator):
    ...
    def get_determined_noise(self, out, batch):
        ...
        corner_idx: Int[Tensor, "H+1 W+1 2"] = torch.stack(
            torch.meshgrid(corner_line_idx_h, corner_line_idx_w, indexing="ij"), dim=-1
        )
        ...
        if self.cfg.smooth_regularization:
            ...
            if s_h % 2 == 0 and s_w % 2 == 0:
                mean_pool_pix = nn.AvgPool2d(
                    (s_h, s_w), (s_h, s_w), padding=(s_h // 2, s_w // 2)
                )
                opacity_pix_BCHW: Float[Tensor, "1 1 Hi Wi"] = opacity[None].permute(
                    0, 3, 1, 2
                )
                z_mean_pix_BCHW: Float[Tensor, "1 1 Hi Wi"] = z_mean[None].permute(
                    0, 3, 1, 2
                )
                ...
                recp_z_mean_pix_BCHW[~mask_pix_BCHW] = 0.0
                recp_z_mean_pix_BCHW = mean_pool_pix(recp_z_mean_pix_BCHW)
                area_BCHW = mean_pool_pix(mask_pix_BCHW.float())
                recp_z_mean_pix_BCHW = torch.nan_to_num(
                    recp_z_mean_pix_BCHW / area_BCHW
                )
                opacity_BCHW: Float[Tensor, "1 1 H+1 W+1"] = mean_pool_pix(
                    opacity_pix_BCHW
                )
                z_mean_BCHW: Float[Tensor, "1 1 H+1 W+1"] = torch.nan_to_num(
                    1 / recp_z_mean_pix_BCHW
                )
            else:
                opacity_BCHW: Float[Tensor, "1 1 H+1 W+1"] = (
                    opacity[corner_mask].reshape(1, hc, wc, -1).permute(0, 3, 1, 2)
                )
                z_mean_BCHW: Float[Tensor, "1 1 H+1 W+1"] = (
                    z_mean[corner_mask].reshape(1, hc, wc, -1).permute(0, 3, 1, 2)
                )
            rays_d_BCHW: Float[Tensor, "1 3 H+1 W+1"] = (
                rays_d[corner_mask].reshape(1, hc, wc, -1).permute(0, 3, 1, 2)
            )
            rays_o_BCHW: Float[Tensor, "1 3 H+1 W+1"] = (
                rays_o[corner_mask].reshape(1, hc, wc, -1).permute(0, 3, 1, 2)
            )
            ...
            recp_z_mean_BCHW[~mask_BCHW] = 0.0
            ...
            corner_suf_pts = (
                (rays_o_BCHW + z_mean_BCHW * rays_d_BCHW)
                .permute(0, 2, 3, 1)
                .reshape(-1, 3)
            )
        else:
    ...
    def update_step(
        ...
        self.smooth_kernel_size = int(
            C(self.cfg.smooth_kernel_size, epoch, global_step)
        )
