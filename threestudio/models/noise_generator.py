...
class NoiseGenerator(BaseModule):
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        ...
        if self.training:
            self.noise_background.data = self.noise_background.data * self.weight_decay + torch.randn_like(self.noise_background.data) * (1.0-self.weight_decay**2)**0.5
...
class TriplaneNoiseGenerator(NoiseGenerator):
    ...
    def get_determined_noise(self, out, batch):
        ...
        assert b == 1, "only support for one batch"
        assert h%self.cfg.noise_h==0 and w%self.cfg.noise_w==0, "no support for strange shape"
        ...
        if self.sph_warmup:
            ...
            mean_pool_pix = nn.AvgPool2d((k_h, k_w),(s_h, s_w), padding=(k_h//2, k_w//2))
            rays_o_pix_BCHW: Float[Tensor, "1 3 Hi Wi"] = rays_o[None].permute(0,3,1,2)
            rays_d_pix_BCHW: Float[Tensor, "1 3 Hi Wi"] = rays_d[None].permute(0,3,1,2)
            
            rays_o_mean_BCHW: Float[Tensor, "1 3 H+1 W+1"] = mean_pool_pix(rays_o_pix_BCHW)
            rays_d_mean_BCHW: Float[Tensor, "1 3 H+1 W+1"] = mean_pool_pix(rays_d_pix_BCHW)
            
            neg_b: Float[Tensor, "1 1 H+1 W+1"] = - (rays_o_mean_BCHW * rays_d_mean_BCHW).sum(dim=1, keepdim=True)
            neg_b_mask = neg_b > 0.
            b2_minus_4a_dot_c: Float[Tensor, "1 1 H+1 W+1"] = neg_b**2 - (rays_o_mean_BCHW**2).sum(dim=1, keepdim=True) + self.cfg.sph_warmup_radius**2
            b2_minus_4a_dot_c_mask = b2_minus_4a_dot_c > 0.
            
            corner_suf_pts: Float[Tensor, "(H+1)(W+1) 3"] = torch.nan_to_num(
                rays_o_mean_BCHW + rays_d_mean_BCHW * (neg_b - b2_minus_4a_dot_c.sqrt())
            ).permute(0,2,3,1).reshape(-1,3)
            corner_opacity: Float[Tensor, "(H+1)(W+1) 1"] = torch.logical_and(
                neg_b_mask,
                b2_minus_4a_dot_c_mask
            ).float().permute(0,2,3,1).reshape(-1,1)
            ...
            point_pix_BCHW: Float[Tensor, "1 3 Hi Wi"] = suf_pts[None].permute(0,3,1,2)
            mask_pix_BCHW: Bool[Tensor, "1 1 Hi Wi"] = opacity_pix_BCHW > self.cfg.opc_thres
            
            # ignore background points when averaging
            point_pix_BCHW[~mask_pix_BCHW.repeat(1,3,1,1)] = 0.
            ...
            corner_opacity: Float[Tensor, "(H+1)(W+1) 1"] = opacity_BCHW.permute(0,2,3,1).reshape(-1,1)
            corner_suf_pts: Float[Tensor, "(H+1)(W+1) 3"] = point_mean_BCHW.permute(0,2,3,1).reshape(-1,3)
        else:
    ...
    def update_step(
        ...
        if self.training:
            # SDE noise injection
            self.noise_background.data = self.noise_background.data * self.weight_decay + torch.randn_like(self.noise_background.data) * (1.0-self.weight_decay**2)**0.5
            self.noise_ptc_val.data = self.noise_ptc_val.data * self.weight_decay + torch.randn_like(self.noise_ptc_val.data) * (1.0-self.weight_decay**2)**0.5