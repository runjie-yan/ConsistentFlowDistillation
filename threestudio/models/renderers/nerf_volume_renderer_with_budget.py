...
@threestudio.register("nerf-volume-renderer-with-budget")
class NeRFVolumeRendererWithBudget(VolumeRenderer):
    @dataclass
    class Config(VolumeRenderer.Config):
        num_samples_per_ray: int = 512
        eval_chunk_size: int = 10_000
        # for memory, set >0 to limit query num
        train_gradient_budget: int = 50_000_000
    ...
    def flatten_forward(
        ...
        **kwargs,
        ...
        if self.training and chunk_budget is not None:
            # compute mask
            ...
            for key in geo_out:
                if torch.is_tensor(geo_out[key]):
                    if geo_out[key].shape[0] == chunk_budget:
                        new_out = torch.zeros(
                            query_pos_num,
                            *geo_out[key].shape[1:],
            ...
            new_rgb_fg_all = torch.zeros(
                query_pos_num, *rgb_fg_all.shape[1:], device=rgb_fg_all.device
            )
                ...
                out.update(
                    {
                        ...
                        "t_dirs": t_dirs,
                        "points": positions,
                        # "ray_indices": ray_indices, # can be decrapted after chunk batch
                        **geo_out,
                    }
                )
